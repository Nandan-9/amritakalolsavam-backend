...
from registrations.models import EventRegistration, GroupParticipants
from events.models import Event
from registrations.validater import can_user_register
from core.helper.get_user_by_id import get_user_by_id
from students.helpers.get_students import validate_same_house
from  registrations.helpers.get_chestnumber import generate_chest_number
from ..permisions import IsRegistration


class EventSpotRegistrationView(APIView):
    ...
    def post(self, request, event_id):
        ...
        # 🔹 Leader limit check
        leader = get_user_by_id(registered_by)
        if not leader  :
                return Response(
                {"error": "Some of the users have not registered"},
                status=400
            )
